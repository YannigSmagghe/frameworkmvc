...
function clone() {
    ...
    // console.log($("#clonedInput"+cloneIndex).find('label'));
    var objectClone = $("#clonedInput" + cloneIndex);
    objectClone.find('label')
        .html('Attribute ' + cloneIndex + ' : ');
    objectClone.find('input')
        .val('')
        .attr('name','c'+cloneIndex);
    objectClone.find('select')
        .attr('name','c'+cloneIndex+'_type'+cloneIndex);

}

function remove() {
    if ($(".clonedInput:last-child").attr("id") !== "clonedInput1") {
        cloneIndex--;
